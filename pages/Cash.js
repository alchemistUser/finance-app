...
const Cash = () => {
  ...
  const [cashBalance, setCashBalance] = useState(0); // State for the cash balance
  ...
  const readTransactions = async () => {
    try {
      ...
      // Filter transactions for selectedAccount === 'Cash'
      const filteredTransactions = combinedTransactions.filter(transaction => transaction.selectedAccount === 'Cash');
      ...
      // Calculate the balance based on income and expense transactions for the 'Cash' account
...
const handleDelete = async (transactionId, source) => {
    try {
      ...
      setTransactions(prevTransactions => {
        ...
        // updateHomeBalance(recalculatedBalance);
        updateBalance();
  ...
  // Function to update the balance in Home.js
  const updateHomeBalance = (balance) => {
    // Implement the method to update the Home.js balance, possibly via context or a state update
    // Example: props.updateHomeBalance(balance); if using props for communication between components
  ...
  return (
    <View style={styles.container}>
      ...
      {transactions.length === 0 ? (
        <Text style={styles.emptyMessage}>No transactions found for Cash account.</Text>
      ) : (
        <FlatList
          data={transactions}
