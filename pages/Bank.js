...
const Bank = () => {
    ...
    const [bankBalance, setBankBalance] = useState(0); // State for the bank balance
    ...
    const readTransactions = async () => {
        try {
            ...
            // Filter transactions for selectedAccount === 'Bank'
            const filteredTransactions = combinedTransactions.filter(transaction => transaction.selectedAccount === 'Bank');
            ...
            // Calculate the balance based on income and expense transactions for the 'Bank' account
    ...
    const handleDelete = async (transactionId, source) => {
        try {
            ...
            setTransactions(prevTransactions => {
                ...
                // Update the bank balance state
                setBankBalance(recalculatedBalance);
    ...
    return (
        <View style={styles.container}>
            <View style={styles.balanceContainer}>
                ...
                <Text style={styles.balanceValue}>${bankBalance.toFixed(2)}</Text>
            </View>
            {transactions.length === 0 ? (
                <Text style={styles.emptyMessage}>No transactions found for Bank account.</Text>
            ) : (
                <FlatList
                    data={transactions}
